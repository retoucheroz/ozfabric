        ...
        try {
            ...
            const response = await fetch("/api/generate", {
                ...
                body: JSON.stringify({
                    ...
                    angleId: targetView, // This is the new line
                    selectedMoodId,
                    hairBehindShoulders,
                    lookAtCamera,
                    enableWind,
                    isStylingShot: !isThreeAngles && (poseFocus as string) !== 'closeup' && !(targetView && targetView.startsWith('std_tech_')),
                    shotIndex: stylingIteration + 1,
                    shotRole: (isThreeAngles || (poseFocus as string) === 'closeup' || (targetView && targetView.startsWith('std_tech_'))) ? 'technical' : 'styling',
                    buttonsOpen,
                    tucked,
                    sleevesRolled,
                    socksType,
